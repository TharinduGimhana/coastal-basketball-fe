...
const DataTable = (props) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                return record.image_path_status == true ? "Visible" : "No Visible"
            }
        },
        {
            title: 'Title',
        ...
        },
        {
            ...
            dataIndex: 'content',
            render: (text, record) => {
                return <Tooltip title={text || 'No content available'}>
                    <span>{text}</span>
                </Tooltip>
            }
        },
        {
            title: 'Logo', dataIndex: 'logo_path', render: (record) => {
                return <img
                    src={`${process.env.REACT_APP_API_BASE_URL}uploads/media/${record}`}
                    style={{ width: '50px', height: '50px', objectFit: 'cover', marginRight: '8px' }}
                />
        ...
        },
        {
            ...
            render: (text, record) => {
                return record.logo_path_status == true ? "Visible" : "No Visible"
            }
        },
