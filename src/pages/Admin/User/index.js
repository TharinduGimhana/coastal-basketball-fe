...
import { UrlAdminBuilkDeleteUser, UrlAdminDeleteUser, UrlAdminGetUsers } from 'ajax/apiUrls';
...
const User = ({ history }) => {
  ...
  const fetchData = async () => {
    ...
    apiPost(UrlAdminGetUsers, formData)
      .then((res) => {
        const fetchedData = res
          .filter(item => item.is_deleted != 1)
          .map((item, index) => ({
            ...item,
            key: item.user_type == "individual" ? "parent" : "child" + "_" + String(item.id),
          }));
